...
export default cds.service.impl(function () {
    ...
    this.before('CREATE', 'Orders', async req => {
        ...
        for (const it of items) {
            ...
            if (!it.quantity || it.quantity < 1) return req.error(400, 'Each item needs a positive quantity');
        ...
        for (const [bookID, qty] of Object.entries(qtyByBook)) {
            const book = await cds.tx(req).run(
                SELECT.one.from('my.bookshop.Books').where({ ID: bookID })
            );
            ...
            if (!affected) {
                return req.error(400, `Not enough stock for book ${bookID}. Requested: ${qty}`);
            }
    ...
    this.on('cancelOrder', async req => {
        const order = await cds.tx(req).run(
            SELECT.one.from(req.subject).columns(o => {
                o.ID, o.isCancelled,
                o.items(i => { i.book_ID, i.quantity })
            })
        );
    ...
    this.on('READ', 'Suppliers', async req => {
        try {
            ...
            return req.error(502, 'Business Partner service is currently unavailable');
        }
    });
});